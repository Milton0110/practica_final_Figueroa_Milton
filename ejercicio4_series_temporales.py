...
def visualizar_serie(serie):
    """
    Genera y guarda un gr?fico de la serie temporal completa.
    ...
    - A?ade t?tulo, etiquetas de ejes y una cuadr?cula suave
...
def descomponer_serie(serie):
    ...
    statsmodels.tsa.seasonal.seasonal_decompose y guarda el gr?fico.
    ...
    - resultado.plot() genera los 4 subgr?ficos autom?ticamente
...
def analizar_residuo(residuo):
    ...
    Genera:
      ...
      - output/ej4_analisis.txt          ? Estad?sticos num?ricos

    Par?metros
    ----------
    residuo : pd.Series ? Componente de residuo de la descomposici?n

    Pistas para el an?lisis num?rico
    ----------------------------------
    - Media:     residuo.mean()
    - Std:       residuo.std()
    - Asimetr?a: residuo.skew()       (ruido ideal ? 0)
    ...
    if residuo_limpio.empty:
        raise ValueError("El residuo queda vac?o tras eliminar NaN.")

    # TODO: Calcula estad?sticos b?sicos
    ...
    # p_adf = resultado_adf[1]
    resultado_adf = adfuller(residuo_limpio)
    p_adf = float(resultado_adf[1])

    # TODO: Gr?fico ACF y PACF del residuo ? output/ej4_acf_pacf.png
    fig_acf_pacf, axes = plt.subplots(2, 1, figsize=(10, 7))
    ...
    fig_acf_pacf.savefig("output/ej4_acf_pacf.png", dpi=150, bbox_inches="tight")
    plt.close(fig_acf_pacf)

    # TODO: Histograma del residuo con curva normal superpuesta
    # ? output/ej4_histograma_ruido.png
    # Pista: usa scipy.stats.norm.pdf para la curva te?rica
    ...
    fig_hist.savefig("output/ej4_histograma_ruido.png", dpi=150, bbox_inches="tight")
    plt.close(fig_hist)
    ...
    with open("output/ej4_analisis.txt", "w", encoding="utf-8") as f:
        ...
        f.write(f"Desviacion tipica: {std:.6f}\n")
        f.write(f"Asimetria: {asimetria:.6f}\n")
        f.write(f"Curtosis (exceso): {curtosis:.6f}\n")
        f.write(f"Jarque-Bera stat: {float(jb_stat):.6f}\n")
        f.write(f"Jarque-Bera p-value: {float(jb_p):.6f}\n")
        f.write(f"ADF stat: {float(resultado_adf[0]):.6f}\n")
        f.write(f"ADF p-value: {p_adf:.6f}\n")
        f.write("\nInterpretacion automatica:\n")
        f.write(
            "- Normalidad: "
            + ("no se rechaza" if float(jb_p) > 0.05 else "se rechaza")
            + " (alpha=0.05)\n"
        )
        f.write(
            "- Estacionariedad (ADF): "
            + ("se rechaza raiz unitaria" if p_adf < 0.05 else "no se rechaza raiz unitaria")
            + " (alpha=0.05)\n"
        )
